...
Location[] LocationsArray = [.. db.Locations];
...
while (eventDate < localDate)
{
  ...
  SortedList<DateTime, Event> dailyEvents = [];
  ...
  for (int i = 0; i < num; i++)
  {
    ...
    int loc = rnd.Next(0, LocationsArray.Length);
    // create date/time for event
    DateTime d = new(eventDate.Year, eventDate.Month, eventDate.Day, hour, minute, second);
    // create event from date/time and location
    Event e = new() { Flagged = false, Location = LocationsArray[loc], LocationId = LocationsArray[loc].LocationId, TimeStamp = d };
